...
void raster ( const SubHeader2 * SH2, const ColorBuffer * CB, DMA *dma )
{
    ...
    volatile RGBA *dma_mem = (RGBA *)(mb2_dmamem0_BASEADDR + sizeof(int)); 
    //DMA *dma = hw_get_dma(2);
    // mk_mon_debug_info(goodrows);
    // mk_mon_debug_info(goodcolumns);
    for ( i = 0; i < goodrows; i++ ) {
        ...
        for ( j = 0; j < goodcolumns; j++ ) {
            ...
            if(dmaAdressCounter == 32){
                hw_dma_send(&mem[lastRowOffset + lastColOffset] , &dma_mem[0], dmaAdressCounter, dma, NON_BLOCKING);
                dmaAdressCounter = 0;
                checkOffsets = 1;
            } 
        }
