...
{
    // DMA dma = {0, mb3_dma1_BASEADDR };
    // // Make sure that DMAmem is not overwritten in raster function, so using higher address
    // volatile unsigned int *dma_mem = (unsigned int *)mb3_dmamem1_BASEADDR; 
    DMA dma = {0, mb3_dma1_BASEADDR };
    // Make sure that DMAmem is not overwritten in raster function, so using higher address
    volatile unsigned int * dma_mem[2] = {mb3_dmamem1_BASEADDR, mb3_dmamem1_BASEADDR + 0x100}; 
