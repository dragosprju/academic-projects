...
namespace SocketServer
{
	...
	public class Form1 : System.Windows.Forms.Form
	{
		...
		public  List<Socket> m_socWorkers = new List<Socket>();
        ...
        private void AddText(string text)
        {
            ...
            {
                this.txtDataRx.Text += text;
                txtDataRx.SelectionStart = txtDataRx.TextLength;
        ...
        private void cmdListen_Click(object sender, System.EventArgs e)
		{
			...
			{
				MessageBox.Show ("cmdListen_Click: " + se.Message );
		...
		public void OnClientConnect(IAsyncResult asyn)
		{
			try
			{
				m_socWorkers.Add(m_socListener.EndAccept(asyn));
				 
				WaitForData(m_socWorkers[m_socWorkers.Count - 1]);

                if (m_socWorkers[m_socWorkers.Count - 1].Connected == true)
                {
                    string connected_text = "Client #" + m_socWorkers.Count.ToString() + " has connected." + Environment.NewLine;

                    AddText(connected_text);
                    byte[] byData = System.Text.Encoding.ASCII.GetBytes(connected_text);
                    for (int j = 0; j < m_socWorkers.Count; j++)
                    {
                        m_socWorkers[j].Send(byData);
                    }
                }
		...
		public  void OnDataReceived(IAsyncResult asyn)
		{
			try
			{
                ...
                for (int i = 0; i < m_socWorkers.Count; i++)
                {
                    if (m_socWorkers[i].GetHashCode() == theSockId.thisSocket.GetHashCode())
                    {
                        AddText("Client #" + (i + 1).ToString() + ": " + szData);
                        AddText(Environment.NewLine);

                        //Object objData = txtDataTx.Text;
                        byte[] byData = System.Text.Encoding.ASCII.GetBytes("Client #" + (i + 1).ToString() + ": " + szData + Environment.NewLine);
                        for (int j = 0; j < m_socWorkers.Count; j++)
                        {
                            m_socWorkers[j].Send(byData);
                        }
			...
			{
                if (!se.Message.Contains("forcibly closed"))
                {
                ...
                else
                {
                    for (int i = 0; i < m_socWorkers.Count; i++)
                    {
                        if (m_socWorkers[i].Connected == false)
                        {
                            string disconnected_text = "Client #" + (i + 1).ToString() + " has disconnected!" + Environment.NewLine;
                            AddText(disconnected_text);
                            m_socWorkers[i].Close();
                            m_socWorkers.RemoveAt(i);

                            byte[] byData = System.Text.Encoding.ASCII.GetBytes(disconnected_text);
                            for (int j = 0; j < m_socWorkers.Count; j++)
                            {
                                m_socWorkers[j].Send(byData);
                            }
		...
		private void buttonSend_Click(object sender, System.EventArgs e)
		{
			...
			{
                Object objData = txtDataTx.Text;
                string server_text = "Server: " + objData.ToString() + Environment.NewLine;

                AddText(server_text);
                byte[] byData = System.Text.Encoding.ASCII.GetBytes(server_text);

                for (int i = 0; i < m_socWorkers.Count; i++)
                {                    
                    m_socWorkers[i].Send(byData);
                }
            }
			catch(SocketException se)
			{
				MessageBox.Show ("buttonSend_Click: " + se.Message );
