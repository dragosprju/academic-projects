...
public class Nava {
	...
	private boolean eJucator;
	
	public Nava(Plansa plansa, boolean eJucator) {
		randomizer = new Random();
		tip = randomizer.nextInt(5) + 1;
		this.eJucator = eJucator;
		foc = new FocBila(plansa, eJucator);
		this.plansa = plansa;
	}
	...
	public void schimbaFoc() { 
		if (foc instanceof FocBila) {
			foc = new FocLinie(plansa, eJucator);
		}
		else if (foc instanceof FocLinie) {
			foc = new FocBila(plansa, eJucator);
		}
	}
