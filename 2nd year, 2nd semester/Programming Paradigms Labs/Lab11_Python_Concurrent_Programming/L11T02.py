...
class ThreadPool(Thread):
	...
	def stop(self):
		self._started = False
		print "Stopping thread pool " + str(self.secondsSharing) + "\n"
		...
		while self._started:
			if self.threads.empty():
				...
				if runningThread.getCredit() == -1:
					...
					self.nextThreadPool.readdTask(runningThread, False)
				else:
...
def main():
	...
	threadPool = ThreadPool(5, 2, threadPool2)
	counters = (5, 20)
	for i in range(2):
